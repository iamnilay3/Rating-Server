...
// To test some commands use the function commandTesting(int paramRatingServerSocketFd, int paramGameServerSocketFd, int paramGameClientRoleNr)
...
void commandTesting(int paramRatingServerSocketFd, int paramGameServerSocketFd, int paramGameClientRoleNr);

int main(int argc, char *argv[])
{
	int gameClientRoleNumber;
	
	int ratingServerSocketFd, gameServerSocketFd;
	...
	if (argc != 6)
	{
		cout << "Usage: ExampleClient GameServerHostname GameServerPortNr RatingServerHostname RatingServerPortNr GameClientRoleNr"
			<< endl << endl;
		return 1;
	}
	
	gameClientRoleNumber = ((int) argv[5][0]) - 48;
	...
	if (gameClientRoleNumber > 0)
	{
		...
		for(p = servinfo; p != NULL; p = p->ai_next)
		{
			if ((gameServerSocketFd = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) == -1)
			{
				perror("client: socket");
				continue;
			}
			
			if (connect(gameServerSocketFd, p->ai_addr, p->ai_addrlen) == -1)
			{
				close(gameServerSocketFd);
				perror("client: connect");
				continue;
			}
	...
	{
		gameServerSocketFd = 0;
	}
	
	commandTesting(ratingServerSocketFd, gameServerSocketFd, gameClientRoleNumber); // Test some commands
...
void commandTesting(int paramRatingServerSocketFd, int paramGameServerSocketFd, int paramGameClientRoleNr)
{
	...
	if (paramGameClientRoleNr == 0)
	{
		...
		strcpy(sendBuffer, sendString.c_str());
		sendCommand(paramRatingServerSocketFd, sendBuffer, 9);
		
		// Protocol Send-Syntax:	10:Nickname
		
		cout << "Info Requesting - Player Info command sent - waiting for server answer..." << endl << endl;
		
		// Protocol Receive-Syntax:	11:{'s','f'}FirstName'\0'SecondName'\0'ThirdName'\0'{'t','f'}NrOfDescriptionCommandsThatWillFollow
		...
		sendString = "003151";
		strcpy(sendBuffer, sendString.c_str());
		sendCommand(paramRatingServerSocketFd, sendBuffer, 6);
		...
		sendBuffer[3] = '2';
		sendCommand(paramRatingServerSocketFd, sendBuffer, sendString.length());
	...
	if (gameClientRoleNumber = 1)
	{
		...
		sendString = "00200";
		strcpy(sendBuffer, sendString.c_str());
		sendCommand(paramgameServerSocketFd, sendBuffer, 5);
		...
		lengthOfReceivedSequence = fetchSequence(paramgameServerSocketFd, receiveBuffer);
